...
# -----------------------------------------------------------------
def parse_args():
    parser = argparse.ArgumentParser(description="GPU (FP32) Layer-wise Profiler")
...
def main():
    ...
    print('-'*20)
    print(f"실험 {args.name}의 GPU (FP32) 레이어 프로파일링 시작")
    print(f"  - Test Image Dir: {args.test_image_dir}")
    print('-'*20)

    # --- 모델 로드 ---
    model = get_model(config).cuda().eval()
    ...
    # --- 프로파일링 ---
    # 1. Warmup (프로파일러 오버헤드 제외)
    print("Warming up GPU...")
    loader_iter = iter(test_loader)
    for _ in range(5): # 5번 예열
        try:
            input_tensor = next(loader_iter).cuda()
        except StopIteration:
            print("예열 중 데이터셋 끝. 프로파일링을 위해 로더 재시작.")
            loader_iter = iter(test_loader)
            input_tensor = next(loader_iter).cuda()
    ...
    # 2. Profiler 실행
    # with_stack=True : 파이썬 스택(어느 레이어인지) 추적
    # record_shapes=True : 텐서 모양 기록
    with torch.profiler.profile(
        activities=[
            torch.profiler.ProfilerActivity.CPU,
            torch.profiler.ProfilerActivity.CUDA,
        ],
        schedule=torch.profiler.schedule(wait=1, warmup=1, active=3, repeat=2), # 2*5=10회 측정
        on_trace_ready=torch.profiler.tensorboard_trace_handler(f'./log/{args.name}_gpu'),
        record_shapes=True,
        with_stack=True,
        profile_memory=True
    ) as prof:
        ...
        for i in tqdm(range(10), desc="Profiling"): # 총 10 스텝 실행
            try:
                input_tensor = next(loader_iter).cuda()
            except StopIteration:
            ...
            prof.step() # 프로파일러에 스텝 알림

    print("\nProfiling complete.")
    
    # 3. 콘솔에 요약 테이블 출력
    print("---[ GPU 연산 시간 Top 20 (Self CUDA Time) ]---")
    # self_cuda_time_total: 커널이 순수하게 GPU에서 실행된 시간 기준 정렬
    ...
    # 4. JSON 트레이스 파일 저장
    trace_path = f"{args.name}_gpu_profile.json"
