...
try:
    ...
    print("경고: Intel Extension for PyTorch (IPEX)를 찾을 수 없습니다.")
# ----------------------------
...
# -----------------------------------------------------------------
def parse_args():
    parser = argparse.ArgumentParser(description="CPU (INT8) Layer-wise Profiler")
...
def main():
    ...
    if not IPEX_AVAILABLE:
        print("오류: Intel Extension for PyTorch (IPEX)가 설치되지 않았습니다.")
        print("이 스크립트를 실행하려면 IPEX를 설치해야 합니다.")
        return
    ...
    print('-'*20)
    print(f"실험 {args.name}의 CPU (INT8) 레이어 프로파일링 시작")
    ...
    print("Calibrating model with 10 samples from test loader...")
    calib_data = []
    loader_iter_calib = iter(test_loader)
    for i in range(10):
        try:
            input_tensor = next(loader_iter_calib)
        except StopIteration:
            if i == 0:
                print("오류: 보정 데이터를 로드할 수 없습니다.")
                return
            else:
                print(f"보정 데이터가 {i}개만 로드되었습니다.")
                break
    ...
    model = convert(prepared_model) # model이 INT8로 변환됨
    print("INT8 Quantization applied.")
    # -----------------------------

    # --- 4. 프로파일링 ---
    # 1. Warmup (INT8 모델)
    print("Warming up CPU (INT8 Model)...")
    ...
    for _ in range(5): 
        ...
        with torch.no_grad():
            _ = model(input_tensor.to(memory_format=torch.channels_last))
    ...
    with torch.profiler.profile(
        ...
        for i in tqdm(range(10), desc="Profiling (INT8)"):
            try:
                input_tensor = next(loader_iter).cpu()
            except StopIteration:
            ...
            with torch.no_grad():
                # [중요] INT8 모델은 channels_last 입력 형식을 기대함
                _ = model(input_tensor.to(memory_format=torch.channels_last))
    ...
    # 3. 콘솔에 요약 테이블 출력
    print("---[ CPU (INT8) 연산 시간 Top 20 (Self CPU Time) ]---")
    # self_cpu_time_total: CPU에서 순수하게 실행된 시간 기준 정렬
    print(prof.key_averages(group_by_input_shape=True).table(
        sort_by="self_cpu_time_total", row_limit=20
    ))
    
    print("\n---[ 연산자별 요약 (INT8 Operator Summary) ]---")
    print(prof.key_averages().table(
        sort_by="cpu_time_total", row_limit=20
    ))

    # 4. JSON 트레이스 파일 저장
    trace_path = f"{args.name}_cpu_int8_profile.json"
