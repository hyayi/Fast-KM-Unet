...
def parse_args():
    parser = argparse.ArgumentParser(description="GPU (FP32) Performance Script")
...
def test_gpu_fp32(config, test_loader, model_path):
    print("\n[시작] 1. GPU (FP32) 성능 측정")
    ...
    with torch.no_grad():
        print(f"Testing (FP32) on {len(test_loader)} images...")
        for input_tensor, target_tensor, _ in tqdm(test_loader, desc="GPU FP32"):
    ...
    # 최종 결과 출력
    print_metrics("GPU (FP32)", iou_avg_meter, dice_avg_meter, hd95_avg_meter)
...
def main():
    ...
    print('-'*20)
    print(f"실험 {args.name}의 GPU (FP32) 성능 측정 시작")
    ...
    # --- 테스트 실행 ---
    test_gpu_fp32(config, test_loader, model_path)
