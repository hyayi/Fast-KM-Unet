...
def test_cpu_int8(config, test_loader, model_path, num_threads):
    print("\n[시작] 3. CPU (INT8) 성능 측정")
    ...
    # [수정] 최신 API QConfigMapping 사용
    qconfig_mapping = ipex.quantization.default_static_qconfig_mapping
    
    print("Calibrating model with 10 samples from test loader...")
    calib_data = []
    # DataLoader를 순회하며 10개 샘플 추출 (pin_memory=False이므로 .cpu() 불필요)
    for i, (input_tensor, _, _) in enumerate(test_loader):
        if i >= 10:
            break
    ...
    with torch.no_grad():
        print(f"Testing (INT8) on {len(test_loader)} images...")
        for input_tensor, target_tensor, _ in tqdm(test_loader, desc="CPU INT8"):
            input_tensor = input_tensor.to(memory_format=torch.channels_last)
            # target_tensor는 이미 CPU에 있음
...
def main():
    ...
    test_cpu_int8(config, test_loader, model_path, args.num_threads)
